    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('dashboard.edit_state') }}</h3>
            </div>
                                ...
                                <select name="lang_id" id="lang_id" class="form-control">
                                    ...
                                    @foreach ($languags as $lang)
                                        <option value="{{ $lang->id }}"
                                            {{ $state->data->first()->lang_id == $lang->id ? 'selected' : '' }}>
                                            {{ $lang->code }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control" id="country_id" name="country_id">
                                    ...
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}"
                                            ...
                                            {{ $country->data->where('lang_id', $lang_id)->first() != null ? $country->data->where('lang_id', $lang_id)->first()->name : $country->data->first()->name }}
                                        </option>
                                    @endforeach
                                </select>
