    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('dashboard.edit_values_services') }}</h3>
            </div>
            <div class="block-content">
                <form action="{{ route('admin.value-services.update', $valuesService->id) }}" method="POST"
                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control" id="title" name="title"
                                       placeholder="{{ __('dashboard.title') }}" value="{{ $valuesService->data->where('lang_id', $lang_id)->first() != null?
                            ...
                            <div class="mb-4">
                                ...
                                @foreach($valuesService->media as $media)
                                    <img src="{{ asset('storage/' . $media->path) }}" alt="
                                     {{ $valuesService->data->where('lang_id', $lang_id)->first() != null?
                                $valuesService->data->where('lang_id', $lang_id)->first()->title :
                                $valuesService->data->first()->title }}" width="100px" height="100px" class="img-thumbnail" style="margin: 10px"/>
                                @endforeach
                                @if ($errors->has('image'))
                                    <span class="text-danger invalid-feedback" role="alert">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="checkbox" id="status" name="status" placeholder="{{ __('dashboard.status') }}
                                    " value="1"
                                    {{ $valuesService->status == 1 ? 'checked' :'' }}>
