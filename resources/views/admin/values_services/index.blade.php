    ...
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">
                {{ __('dashboard.qaysegp') }} <small>{{ __('dashboard.values_services') }}</small>
            </h3>
        </div>
        <div style="margin: 10px 10px 0 0;">
        @can('create_slider')
                    <a class="btn btn-primary m-2"
                        href="{{ route('admin.value-services.create') }}">{{ __('dashboard.add_values_services') }}</a>
                @endcan
            ...
            <table class="table table-bordered table-striped table-vcenter js-dataTable-buttons">
                ...
                <tbody>
                    @foreach ($valuesServices as $valuesService)
                        <tr>
                            <td class="text-center">{{ $valuesService->id }}</td>
                            ...
                            <td class="text-center">
                                @can('show_slider')
                                    <button class="btn btn-success">
                                        <a href="{{ route('admin.value-services.show', [$valuesService->id]) }}"
                                ...
                                @can('edit_slider')
                                    <button class="btn btn-info">
                                        <a href="{{ route('admin.value-services.edit', [$valuesService->id]) }}"
                                            class="text-secondary font-weight-bold text-xs" data-toggle="tooltip"
                                ...
                                @endcan
                                @can('delete_slider')
                                    <form action="{{ route('admin.value-services.destroy', [$valuesService->id]) }}" method="post"
