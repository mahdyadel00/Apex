    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('dashboard.edit_sector') }}</h3>
            </div>
                                ...
                                <select name="lang_id" id="lang_id" class="form-control">
                                    ...
                                    @foreach ($languages as $lang)
                                        <option value="{{ $lang->id }}"
                                            {{ $sector->data->first()->lang_id == $lang->id ? 'selected' : '' }}>
                                            {{ $lang->code }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control" id="state_id" name="state_id">
                                    ...
                                    @foreach ($states as $state)
                                        <option value="{{ $state->id }}"
                                            {{ $sector->state_id == $state->id ? 'selected' : '' }}>
                                            {{ $state->data->where('lang_id', $lang_id)->first() != null ? $state->data->where('lang_id', $lang_id)->first()->name : $state->data->first()->name }}
                                        </option>
                                    @endforeach
                                </select>
